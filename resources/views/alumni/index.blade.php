...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">Kelola data alumni yang akan dilacak</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('export', request()->all()) }}"
                class="inline-flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-900/50 transition-colors">
    ...
    <div id="import-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 hidden">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6 mx-4">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Import Alumni (CSV)</h3>
                <button onclick="document.getElementById('import-modal').classList.add('hidden')" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-300">&times;</button>
            </div>
            <form action="{{ route('alumni.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div class="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-lg p-8 text-center hover:border-blue-400 transition-colors cursor-pointer" 
                    onclick="document.getElementById('csv-file').click()">
                    <input type="file" name="file" id="csv-file" class="hidden" onchange="updateFileName(this)">
                    <p id="file-name" class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">Klik untuk memilih file CSV</p>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Format: NIM, Nama, Prodi, Tahun Lulus</p>
                </div>
                <div class="flex gap-2">
                    <button type="button" onclick="document.getElementById('import-modal').classList.add('hidden')"
                        class="flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                        Batal
                    </button>
        ...
        <form method="GET" action="{{ route('alumni.index') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                    placeholder="Cari NIM, nama, atau prodi...">
            </div>
            <select name="status"
                class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white dark:bg-gray-800">
            ...
            <div class="flex gap-2">
                ...
                @if (request()->hasAny(['search', 'status', 'prodi', 'tahun_lulus', 'per_page']))
                    <a href="{{ route('alumni.index') }}"
                        class="px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                        ...
                        @endphp
                        <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">
                        ...
                        <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Prodi</th>
                        <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">
                        ...
                        <th class="px-5 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($alumni as $alum)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-900/50 transition-colors">
                            <td class="px-5 py-3 font-mono text-gray-600 dark:text-gray-300 whitespace-nowrap">{{ $alum->nim }}</td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('alumni.show', $alum->nim) }}"
                                        class="text-gray-400 dark:text-gray-500 hover:text-blue-600 transition-colors" title="Detail">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('alumni.edit', $alum->nim) }}"
                                        class="text-gray-400 dark:text-gray-500 hover:text-yellow-600 transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <form method="POST" action="{{ route('alumni.destroy', $alum->nim) }}"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-gray-400 dark:text-gray-500 hover:text-red-600 transition-colors"
                                            title="Hapus">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-gray-400 dark:text-gray-500 text-sm">Belum ada data alumni.</p>
                                    <a href="{{ route('alumni.create') }}"
